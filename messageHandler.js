...
const handleMessages = async (message) => {
  ...
  if (message.channel.id === monitoredChannelId) {
    ...
    if (message.attachments.size === 0 && !hasAdminRole) {
      message.delete()
        .then(() => console.log(`Deleted message from channel ${monitoredChannelId}`))
        .catch(console.error);
    }
  }
  ...
  if (/chino/i.test(message.content)) {
    const attachment = new AttachmentBuilder('emote.jpg');
  ...
  if (message.content === "pakilinis po") {
    ...
    let deleteCount = await ask("Ilan ba gusto mo burahin? (Max: 99)", m => !isNaN(m.content) && filterByUser(m));
    deleteCount = Math.min(parseInt(deleteCount), 99); 

    const confirmation = await ask(`sure ka buburahin mo ${deleteCount} na mensahe? \n 1 : yes \n 2 : no`, m => ['1', '2'].includes(m.content) && filterByUser(m));
    ...
    if (confirmation === '1') {
      ...
      const tempMessage = await message.channel.send("walang binura na message.");
